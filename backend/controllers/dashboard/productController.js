            ...
            try{
                ...
                if(images.length){
                    for(let i=0; i<images.length; i++){
                        result = await cloudinary.uploader.upload(images[i].filepath, {
                        folder: 'products' 
                        })
                    ...
                    result = await cloudinary.uploader.upload(images.filepath, {
                       folder: 'products' 
                    })
                    allImageUrl = [...allImageUrl, result.url]
                }
        ...
        try{
            if(searchValue){
                const products = await productModel.find({
                    ...
                    sellerId: id
                }).skip(skipPage).limit(perPage).sort({createdAt: -1}) //由新到舊
                ...
                const products = await productModel.find({
                    sellerId: id
                }).skip(skipPage).limit(parseInt(perPage)).sort({createdAt: -1}) //由新到舊
                const totalProduct = await productModel.find({
                    sellerId: id
                }).countDocuments()
                responseReturn(res, 200, { products, totalProduct })                
        ...
        form.parse(req, async (err, field, files) => {
            ...
            if(err){
                ...
                try{
                    ...
                    // 因為他只要更新一張圖，所以不用像 add_product 那樣用迴圈
                    let result = await cloudinary.uploader.upload(newImage.filepath, {
                                 folder: 'products'})
