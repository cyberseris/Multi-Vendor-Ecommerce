...
import { get_sellers, get_admin_message, send_message_seller_admin, updateAdminMessage, updateSellerMessage, messageClear } from '../../store/Reducers/chatReducer'
...
const ChatSeller = () => {
    ...
    useEffect(() => {
        if(successMessage){
            console.log("admin successMessage", seller_admin_message[seller_admin_message.length-1])
    ...
    useEffect(()=>{
        if(receiverMessage){
            if(sellerId===receiverMessage.senderId && receiverMessage.receiverId === ''){
                dispatch(updateSellerMessage(receiverMessage))
            }else{
                toast.success(receiverMessage.sellerName + " " + "Send A Message")
                dispatch(messageClear())
            }
        }
    }, [receiverMessage])
                ...
                <div className='flex w-full h-full relative'>
                    <div className={`w-[280px] h-full absolute z-10 ${show?'-left-[16px]':'-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#9e97e9] md:bg-transparent overflow-y-auto'>
                            ...
                            <div className='flex text-xl justify-between items-center p-4 md:p-0 md:pb-3 text-white'>
                                <h2>Sellers</h2>
                                <span onClick={()=>setShow(!show)} className='block cursor-pointer md:hidden'><IoMdClose /></span>
                            </div>

                            {
                                sellers.map((s,i) => <Link key={i} to={`/admin/dashboard/chat-sellers/${s._id}`} className={`h-[60px] flex justify-start items-center gap-2 text-white px-2 py-2 rounded-md cursor-pointer  ${sellerId === s._id ? 'bg-[#8288ed]': ''}`}>
                                <div className='relative'>
                                    <img className='w-[38px] h-[38px] border-white border-2 max-w-[38px] p-[2px] rounded-full' src={s.image} alt="" />
                                    {
                                        activeSeller.some(a => a.sellerId === s._id) && <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'>
                                        </div>
                                    }
                                </div>
                    ...
                    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
                        <div className='flex justify-between items-center'>
                            ...
                            <div onClick={()=>setShow(!show)} className='w-[35px] h-[35px] flex md:hidden rounded-sm bg-blue-500 shadow-lg hover:shadow-blue-500/50 justify-center cursor-pointer items-center text-white'>
                                <span><FaList /></span>
                            </div>
                        </div>
                                        ...
                                        if(m.senderId !== sellerId){
                                            return (
                                                <div key={i} ref={scrollRef} className='w-full flex justify-start items-center'>
                                                    <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                                        ...
                                                        <div className='flex flex-col justify-center items-start w-full bg-blue-500 shadow-lg shadow-blue-500/50 text-white py-1 px-2 rounded-sm'>
                                                            <span>{m.message}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            return (
                                              <div key={i} ref={scrollRef} className='w-full flex justify-end items-center'>
                                                <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                                    <div className='flex flex-col justify-center items-start w-full bg-red-500 shadow-lg shadow-red-500/50 text-white py-1 px-2 rounded-sm'>
                                                        <span>{m.message}</span>
                                                    </div>
                                                    <div>
                        ...
                        <form onSubmit={send} className='flex gap-3'>
                            ...
                            <button disabled={sellerId ? false : true} className='w-[75px] h-[35px] shadow-lg bg-[#06b6d4] hover:shadow-cyan-500/50 text-semibold rounded-md text-white justify-center items-center'>Send</button>
                        </form>
