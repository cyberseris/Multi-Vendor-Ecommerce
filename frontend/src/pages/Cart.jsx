...
const Cart = () => {
    ...
    const { cart_products, buy_product_item, outofstock_products, price, shipping_fee, successMessage, errorMessage } = useSelector(state => state.cart)
    ...
    const increment = (quantity, stock, cartProductId) => {
        const newQuantity = quantity + 1;

        if(newQuantity <= stock){
            dispatch(quantity_inc(cartProductId))
        }
    }

    const decrement = (quantity, cartProductId) => {
        const newQuantity = quantity - 1;

        if(newQuantity !== 0){
            dispatch(quantity_dec(cartProductId))
        }
    }
                    ...
                    <div className="w-[85%] lg:w-[90%] md:w-[90%] sm:w-[90%] mx-auto py-16">
                        {
                            cart_products?.length > 0 || outofstock_products?.length > 0 ? <div className="flex flex-wrap">
                                <div className="w-[67%] mg-lg:w-full">
                                    <div className="pr-3 md-lg:pr-0">
                                        <div className="flex flex-col gap-3">
                                            <div className="bg-white p-4">
                                                <h2 className="text-md text-green-500 font-semibold">Stock Products {buy_product_item}</h2>
                                            </div>

                                            {
                                                cart_products.map((p,i)=><div key={i} className="flex bg-white p-4 flex-col gap-2">
                                                    <div className="flex justify-start items-center">
                                                        <h2 className="text-md text-slate-600 font-bold">{p.shopName}</h2>
                                                    </div>

                                                    {
                                                        p.products.map((pl,i)=><div key={i} className="w-full flex flex-wrap">
                                                            <div className="flex sm:w-full gap-2 w-7/12 pb-2">
                                                                <div className="flex gap-2 justify-start items-center">
                                                                    <img className="w-[80px] h-[80px]" src={pl.productInfo?.images[0]} alt="" />

                                                                    <div className="pr-4 text-slate-600">
                                                                        <h2 className="text-md font-semibold">{pl.productInfo?.name}</h2>
                                                                        <span className="text-sm">Brand: {pl.productInfo?.brand}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div className="flex justify-between w-5/12 sm:w-full sm:mt-3">
                                                                <div className="pl-4 sm:pl-0">
                                                                    <h2 className="text-lg text-orange-500">${pl.productInfo?.price-pl.productInfo?.discount}</h2>
                                                                    <p className="line-through">${pl.productInfo?.price}</p>
                                                                    <p>-{pl.productInfo?.discount}</p>
                                                                </div>

                                                                <div className="flex gap-2 flex-col">
                                                                    <div className="flex bg-slate-200 h-[30px] justify-center items-center text-xl">
                                                                        <div onClick={()=>decrement(pl.quantity, pl._id)} className="px-3 cursor-pointer">-</div>
                                                                        <div className="px-3 cursor-pointer">{pl.quantity}</div>
                                                                        <div onClick={()=>increment(pl.quantity,pl.productInfo.stock, pl._id)} className="px-3 cursor-pointer">+</div>
                                                                    </div>
                                                                    <button onClick={()=>dispatch(delete_cart_product(pl._id))} className="px-5 py-[3px] bg-red-500 text-white">Delete</button>
                                                                </div>
                                                            </div>
                                                        </div>)
                                                    }
                                                        ...
                                                        outofstock_products?.map((p,i)=><div key={i} className="w-full flex flex-wrap">
                                                            <div className="flex sm:w-full gap-2 w-7/12 pb-2">
                                                                <div className="flex gap-2 justify-start items-center">
                                                                    ...
                                                                    <div className="pr-4 text-slate-600">
                                                                        <h2 className="text-md font-semibold">{p.products[0].shopName}</h2>
                                                            ...
                                                            <div className="flex justify-between w-5/12 sm:w-full sm:mt-3">
                                                                <div className="pl-4 sm:pl-0">
                                                                    <h2 className="text-lg text-orange-500">${p.products[0].price-p.products[0].discount}</h2>
                                    ...
                                    <div className="pl-3 md-lg:pl-0 md-lg:mt-5">
                                        {
                                            cart_products?.length > 0 && <div className="bg-white p-3 text-slate-600 flex flex-col gap-3">
                                                ...
                                                <div className="flex gap-2">
                                                    <input className="w-full px-3 py-2 border border-slate-200 outline-0 focus:border-green-500 rounded-sm" type="text" placeholder="Input Varchar Coupon" />
