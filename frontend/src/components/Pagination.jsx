...
const Pagination = ({ pageNumber, setPageNumber, totalItem, perPage, showItem }) => {
    ...
    const createBton = () => {
        ...
        for(let i = startPage; i < endPage; i++){
            btns.push(
                <li onClick={() => setPageNumber(i)} key={i} className={`${pageNumber === i ? 'bg-green-700 shadow-lg shadow-green-300/50 text-white': 'bg-slate-600 hover:bg-green-400 shadow-lg hover:shadow-indigo-500/50 hover:text-white text-[#d0d2d6]'} 
